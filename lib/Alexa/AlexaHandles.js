...
class AlexaHandles {
    ...
    handleRequest(req) {
        let namespace = (((req.body || {}).directive || {}).header || {}).namespace || 'Alexa';
        namespace = namespace.replace('Alexa.', '').replace(/\./, '');
        let method = (((req.body || {}).directive || {}).header || {}).name || 'Alexa';

        if (typeof (this['handle' + namespace + method]) === "function") {
            return this['handle' + namespace + method](req);
        }
        if (typeof (this['handle' + method]) === "function") {
            return this['handle' + method](req);
        }
        if (typeof (this['handle' + namespace]) === "function") {
            return this['handle' + namespace](req);
        }
        return Promise.reject('Unknown request (namespace: ' + namespace + ' / name: ' + name + ')');
    }

    handleEvent(req) {
        let namespace = ((req.body || {}).header || {}).namespace || 'Alexa';
        namespace = namespace.replace('Alexa.', '').replace(/\./, '');
        let method = ((req.body || {}).header || {}).name || 'Unknown';
        if (typeof (this['handle' + namespace + method]) === "function") {
            return this['handle' + namespace + method](req);
        }
        if (typeof (this['handle' + method]) === "function") {
        ...
        return Promise.reject('Unknown event (namespace: ' + namespace + ' / name: ' + name + ')');
    }
    ...
    handleRTCSessionControllerInitiateSessionWithOffer(req) {
        return new Promise(async (resolve) => {
            ...
            if (!this.rtcSessions[sessionId]) {
                this.rtcSessions[sessionId] = new RTCPeerConnection();
            }
            this.rtcSessions[sessionId].setRemoteDescription(offerValue);
            const answer = await this.rtcSessions[sessionId].createAnswer();
            this.rtcSessions[sessionId].setLocalDescription(offerValue);
            this.rtcSessions[sessionId].addEventListener('icecandidate', (event) => {
                this.adapter.log.info('RTC.icecandidate');
                this.rtcSessions[sessionId].addIceCandidate(event.candidate);
            });
            this.rtcSessions[sessionId].addEventListener('iceconnectionstatechange', (event) => {
                this.adapter.log.info('RTC.iceconnectionstatechange');
            });
            this.rtcSessions[sessionId].addEventListener('track', (event) => {
                this.adapter.log.info('RTC.track');
            });

            setTimeout(() => {
                if (this.rtcSessions[sessionId]) {
                    try {
                        this.adapter.log.info('RTC.close');
                        this.rtcSessions[sessionId].close();
                    } catch (e) {
                        this.adapter.log.warn('RTC.error: ' + e.message);
                    }
                    delete this.rtcSessions[sessionId];
                }
            }, 60_000);
