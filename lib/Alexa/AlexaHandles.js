...
class AlexaHandles {
    ...
    _checkEndpoint(device) {
        try {
            ...
            device.manufacturerName = device.manufacturerName.replace(/[^.a-zA-Z0-9]+/g, 'X');
            if (device.manufacturerName.length > 128) {
                throw new Error('Invalid device manufacturer name.');
            }

            device.description = device.description.replace(/[^.a-zA-Z0-9\s]+/g, 'X');
            if (device.description.length > 128) {
                throw new Error('Invalid device description.');
            }

            device.friendlyName = device.friendlyName.replace(/[^.a-zA-Z0-9]+/g, 'X');
