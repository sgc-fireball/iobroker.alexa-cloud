...
class HmIPSMO extends BaseDevice {

    update(id, state) {
        if (!BaseDevice.prototype.update.call(this, id, state)) {
            return false;
        }
        if (id.indexOf(this.id) === 0 && /\.(MOTION)$/.test(id) && state.val) {
            ...
            this.getAlexaContext()
                ...
                .catch((e) => {
                    this.adapter.log.info('Bewegungsmelder.error: ' + JSON.stringify(e));
                });
        }
        return true;
    }
    ...
    motion() {
        return this.getState(this.id + '.2.MOTION'); // true = DETECTED, false = NOT_DETECTED
    }
