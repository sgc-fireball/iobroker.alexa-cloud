...
class AlexaHandles {
    ...
    handleCameraStreamController(req) {
        return new Promise(async (resolve) => {
            ...
            const streamUrl = await device.getStreamUrl();
    ...
    /**
     * @see https://developer.amazon.com/en-US/docs/alexa/device-apis/alexa-rtcsessioncontroller.html
     */
    handleRTCSessionController(req) {
        return new Promise(async (resolve) => {
            const endpointId = req.body.directive.endpoint.endpointId;
            try {
                jwt.verify(req.body.directive.endpoint.scope.token, this.adapter.config.oauth_client_secret);
            ...
            const config = await device.getConfig();
            throw new Error('NOT SUPPORTED AT THIS MOMENT');
        });
    }
