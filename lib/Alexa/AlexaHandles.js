...
class AlexaHandles {
    ...
    handleCameraStreamController(req) {
        return new Promise(async (resolve, reject) => {
            ...
            const width = req.body.directive.payload.cameraStreams[0].resolution.width;
            const height = req.body.directive.payload.cameraStreams[0].resolution.height;
            ...
            let response = AlexaUtils.event(
                ...
                },
                {
                    ...
                    payload: {
                        cameraStreams: [
                            {
                                uri: httpHost + device.getHighStreamUrl(width, height),
                        ...
                        imageUri: httpHost + device.getSnapshotUrl()
                    }
                }
            );
            response.context = await device.getAlexaContext();
