    ...
    handleReportState(event) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                resolve(AlexaUtils.eventError({}, {endpoint: {endpointId: endpointId}}, 'EXPIRED_AUTHORIZATION_CREDENTIAL', 'Please relogin.'));
                return;
            }
            const devices = this.adapter.getSupportedDevices();
            if (!devices.hasOwnProperty(endpointId)) {
                resolve(AlexaUtils.eventError({}, {endpoint: {endpointId: endpointId}}, 'NO_SUCH_ENDPOINT', 'Unknown device.'));
                return;
            }
            devices[endpointId].getAlexaContext()
                    ...
                    resolve({
                        event: {
                            header: {
                                ...
                                payloadVersion: 3,
                                messageId: uuid(),
                                correlationToken: event.event.header.correlationToken
                            },
                            endpoint: {
                                endpointId: endpointId
                            },
                ...
                .catch((e) => {
                    resolve(AlexaUtils.eventError({}, {endpoint: {endpointId: endpointId}}, 'INTERNAL_ERROR', e));
                });
        });
    }
