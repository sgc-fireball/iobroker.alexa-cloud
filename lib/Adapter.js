...
class Adapter extends CoreUtils.Adapter {
    ...
    registerDevices() {
        return new Promise((resolve) => {
            this.log.info('[registerDevices]: started');
            /**
             * @link views/registerDevices.js
             * @link io-package.json
             */
            this.getObjectView(
                adapterName,
                ...
                (err, devices) => {
                    if (!!err) {
                        this.log.warn('[registerDevices]: ' + err.toString());
                        resolve();
                        return
                    }
                    if (!devices || !Array.isArray(devices.rows) || devices.rows.length === 0) {
                        this.log.info('[registerDevices]: no devices found');
                        resolve();
                        return;
                    }
                    devices.rows.forEach(device => {
            ...
            this.log.info('[registerDevices]: loaded');
        });
    }
