...
class AlexaBridge {
    ...
    handle(request) {
        this.adapter.log.info('AlexaBridge.handle: Request: ' + JSON.stringify(response));
        this.alexaHandles.handle(request)
            .then((response) => {
                this.adapter.log.info('AlexaBridge.handle: Response: ' + JSON.stringify(response));
            })
            .catch((e) => {
                this.adapter.log.warn('AlexaBridge.handle: Error: ' + e);
            });
    }
