...
class AlexaBridge {
    ...
    async _httpRtspSnapshot(req, res, next) {
        ...
        let deviceId = req.params?.stream?.toString() || 'unknown';
        deviceId = 'alexa-cloud.0.onvif.' + deviceId
        this.adapter.log.info('Search camera ' + deviceId);
        const device = this.adapter.getSupportedDevices(deviceId)
        if (device === null) {
            this.adapter.log.warn('Could not found camera: ' + deviceId)
            return res.status(404).end();
        }

        const snapshopUrl = device.getSnapshotUrl();
        if (!snapshopUrl || !snapshopUrl.startsWith('http')) {
