                    ...
                    resolve({
                        event: {
                            header: {
                                ...
                                payloadVersion: 3,
                                messageId: uuid(),
                                correlationToken: rewq.directive.header.correlationToken
                            },
                            endpoint: {
                                endpointId: endpointId
                            },
            ...
            let response = AlexaUtils.event(
                ...
                },
                {
                    ...
                    payload: {
                        cameraStreams: [
                            {
                                uri: httpHost.replace('https', 'rtsp').replace('http', 'rtsp')
                                    + ':443'
                        ...
                        imageUri: httpHost + device.getSnapshotUrl(stream) + '?token=' + req.body.directive.endpoint.scope.token
                    }
                }
            );
            response.context = await device.getAlexaContext();
