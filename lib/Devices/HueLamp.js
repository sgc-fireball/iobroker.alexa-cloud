...
class HueLamp extends BaseDevice {
    ...
    brightness(value = null) {
        if (value === null) {
            return this.getState(this.id + '.action.brightness').then(value => parseInt(value));
        }
        this.setState(this.id + '.action.brightness', parseInt(Math.max(0, Math.min(value, 100))));
    }
