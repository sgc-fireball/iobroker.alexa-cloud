...
class AlexaHttpEndpoint {
    ...
    oauthToken(req, res, next) {
        ...
        if (req.body.grant_type === 'authorization_code') {
            ...
            if (!jwt.verify(req.body.refresh_token, process.env.oauth_client_secret)) {
        ...
        let response = {
            access_token: jwt.sign({id: tokenId, type: 'access_token'}, process.env.oauth_client_secret, {expiresIn: expires_in}),
            token_type: 'bearer',
            expires_in: expires_in,
            refresh_token: jwt.sign({id: tokenId, type: 'refresh_token'}, process.env.oauth_client_secret, {expiresIn: expires_in * 30}),
            scope: 'iobroker',
        };
