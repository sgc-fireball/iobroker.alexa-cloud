...
class AlexaHandles {
    ...
    handleCameraStreamController(req) {
        return new Promise(async (resolve) => {
            ...
            let uri = '';
            uri += (device?.protocol === 'RTSP') ? 'rtsp://' : 'http://';
            if (device.authorizationType !== 'NONE') {
                uri += encodeURIComponent(device.user || device.username) + ':';
                uri += encodeURIComponent(device.pass || device.password) + '@';
            }
            uri += device.ip + ':' + device.port + '/' + device.path;
            ...
            let response = AlexaUtils.event(
                ...
                },
                {
                    ...
                    payload: {
                        ...
                        imageUri: 'https://raw.githubusercontent.com/sgc-fireball/iobroker.alexa-cloud/master/icon.png' // httpHost + device.getSnapshotUrl(stream) + '?token=' + req.body.directive.endpoint.scope.token
                    }
                }
            );
            response.context = await device.getAlexaContext();
