...
class AlexaHandles {
    ...
    handle(request) {
        let messageId = ((((request || {}.directive) || {}).header || {}).messageId || null) || ((((request || {}.event) || {}).header || {}).messageId || null) || null;
        this.adapter.log.info('Request[' + messageId.toString() + ']: ' + JSON.stringify(request));
        if (this.cache.hasOwnProperty(messageId)) {
            this.adapter.log.info('Response[' + messageId.toString() + ']: from cache.');
            return Promise.resolve(this.cache[messageId]);
        }

        let promise = !!request.directive ? this.handleRequest(request) : this.handleEvent(request);
        return promise
            .then((response) => {
                this.adapter.log.info('Response[' + messageId.toString() + ']: ' + JSON.stringify(response));
                this.cache[messageId] = response;
                setTimeout(() => delete this.cache[messageId], 30);
                return response;
            });
    }
